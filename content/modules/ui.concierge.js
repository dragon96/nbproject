...
define(function(require) {
  ...
  NB$.isHTML = function (text) {
    // Returns true if the text represents a html string generated by TinyMCE. This function checks whether
    // the content begins with a html tag
    return /^<[a-z][\s\S]*>/i.test(text.trim());
  };

  NB$.truncateURL = function (str) {
    // Truncate long URLs: If url > 23 characters, only display first 20 characters followed by ...
    // If the original content is plain text, first convert it to html so we can use jQuery find(), then
    // convert it back to text after truncating long URLs.
    var wasOriginallyHtml = $.isHTML(str);
